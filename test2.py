...
import numpy as np
import time
# to run, open a new terminal and write streamlit run (program name)
    ...
    while cap.isOpened():
        ...
        if current_test == -1:
            if time.time() - task_start_time < 5:
                cv2.putText(image, welcome_message, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2, cv2.LINE_AA)
            else:
                current_test += 1
                task_start_time = time.time()
            ...
            if current_test < len(tests):
                ...
                cv2.putText(image, tests[current_test], (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                
                if current_test in [1, 2]:  # Show fist open/close count
                    cv2.putText(image, f'Fist Open/Close Count: {fist_open_close_count // 2}', (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                ...
                cv2.putText(image, result_text, (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
